...
void SatTerminalPhy::allocateFrame(int frameIndex)
{
    ...
    for(int i = 0; i < locationArrays.size(); i++)
    {
        ...
        for(int j = 0; j < locationArrays[i].size() - 1; j++)
        {
            ...
            int temp = returnFrame->getSrcAddress();

            if(temp<1000){
                double N = temp/100+2;
                double channelRatio= basic+  (N*100-temp)/(N*100);
                returnFrame->setChannelRatio(channelRatio);

            }else if(temp>1000){
                int M = temp%1000;
                double N = M/100+2;
                double channelRatio= basic +  (N*100-M)/(N*100);
                returnFrame->setChannelRatio(channelRatio);
            }

            //returnFrame->setChannelRatio(2+0.3);
